    ...
    execute(pedidos: any, notas: any){
        ...
        for(let i= 1; i <= keysPedido.length; i++){
            ...
            pedidos[i - 1][i].forEach((item)=>{
                const valorUnitario = parseFloat(item.valor_unitario_produto.replace(',', '.'));
        ...
        for(let i= 1; i <= keysPedido.length; i++){
            ...
            for(let nt of nota){
                const item = saldos[i].itensPendentes.find(i => i.numero_item === nt.numero_item);
                if (item) {
                    item.quantidade_pendente -= notas[i - 1][i].quantidade_produto;
                    const pedido = pedidos.find((p) => p[i])
                    const valorUnitario = parseFloat(pedido[i].find(p => p.numero_item === item.numero_item).valor_unitario_produto.replace(',', '.'));
                    item.valor_pendente -=  notas[i - 1][i].quantidade_produto * valorUnitario;
                }
            }
        }
        
        Object.keys(saldos).forEach(idPedido => {
            saldos[idPedido].itensPendentes = saldos[idPedido].itensPendentes.filter(item => item.quantidade_pendente > 0);
            saldos[idPedido].saldoValor = saldos[idPedido].itensPendentes.reduce((acc, item) => acc + item.valor_pendente, 0);
        });
